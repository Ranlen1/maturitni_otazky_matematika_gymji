...
\subsection*{Vektor a operace s ním}
...
Tato úsečka se značí jako $\overrightarrow{AB}$. Z orientované úsečky s body $A[x_1, y_1], B[x_2, y_2]$ získáme vektor $\overrightarrow{u} = (u_1, u_2), u_1 = x_2 - x_1, u_2 = y_2 - y_1$. Vektor se dá
brát jako předpis posunutí, tedy o kolik a jakým směrem se musíme posunout z bodu $A$, abychom se
dostali do bodu $B$. Velikost vektoru se získává pomocí vzorce $|\overrightarrow{u}| =
\sqrt{u_1^2 + u_2^2}$. Velikost vektoru odpovídá $|AB|$.

\subsubsection*{Sčítání, odčítání a násobení vektorů realným číslem}
Sčítání vektorů $\overrightarrow{u} = (u_1, u_2), \overrightarrow{v} = (v_1, v_2)$ se dá představit
jako kombinaci těchto dvou posunutí. Místo toho abychom se posunuli nejříve podle prvního vektoru
a následně druhého to skombinujeme do jednoho posunutí, tedy $\overrightarrow{u} + \overrightarrow{v}
= (u_1 + v_1, u_2 + v_2)$.

Odčítání vektorů se dá představit jako sčítání opačného. Mějme dva vektory $\overrightarrow{u} =
(u_1, u_2), \overrightarrow{v} = (v_1, v_2)$. Opačný vektor se tvoří znegováním každé složky vektoru,
tedy opačný vektor je $\overrightarrow{v_o} = (-v_1, -v_2)$. Platí
$\overrightarrow{u} - \overrightarrow{v} = (u_1 - v_1, u_2 - v_2)$ nebo
$\overrightarrow{u} + \overrightarrow{v_o} = (u_1 - v_1, u_2 - v_2)$.

Pokud chceme vynásobit vektor realným číslem $k$, tímto číslem si vynásobí každá složka daného
vektoru. Tedy $k \cdot \overrightarrow{u} = (k \cdot u_1, k \cdot u_2)$.
